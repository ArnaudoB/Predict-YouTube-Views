from models.ResBert.ResBert import ResBert
from models.ResBert.ResBertDataset import ResBertDataset
import torch
    ...
    for epoch in range(epochs):
        ...
        if early_stopping:
            if avg_val_loss < best_val_loss:
                ...
                torch.save(model.state_dict(), f'./models/ResBert/best_model_ResBert_allsemifrozen256.pth')
            else:
                counter += 1
                if counter >= patience:
...
if __name__ == '__main__':
    ...
    for name, param in model.named_parameters():
        if param.requires_grad:  # Only include trainable parameters
            if name.startswith('7.') or 'encoder.layer.11' in name or 'pooler' in name:
    ...
    optimizer = optim.Adam([
        {'params': low_lr_params, 'lr': 1e-5},  # Fine-tuning lr for encoders
        {'params': high_lr_params, 'lr': 1e-3}  
    ], weight_decay=1e-5)
    ...
    plt.legend()
    plt.savefig('./artifacts/ResBert_allsemifrozen256_plot.png')